...
function load_text_on_canvas(){
	...
	if(responsive){
		...
		console.log("TEST2");
		document.getElementById("pcver1").innerText = `\$\$\\text{Activation Function: } f(x) = \\begin{cases} x \\text{ if } x \\ge 0 \\\\ 0 \\text { if } x \\lt 0 \\end{cases}\\text{, Output Function: } g(x) = \\begin{cases} 1 \\text{ if } x \\ge 0.5 \\\\ 0 \\text{ if } x \\lt 0.5 \\end{cases}\$\$`;
		document.getElementById("mobileonly1").innerText = "";
	}
	MathJax.typeset();
}
